...
/*   Updated: 2022/07/24 01:04:51 by dongkim          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "draw_minimap.h"

static void	init_minimap(t_cub3d *p_data, t_minimap *p_map, unsigned int tile_size)
{
	...
	unsigned int *bpos, unsigned int tile_size)
{
	...
	while (pos[1] < map->height + bpos[1])
	{
		...
		{
			c = content[(pos[1] - bpos[1]) / tile_size][(pos[0] - bpos[0]) / tile_size];
			if (c == 1)
	...
	unsigned int *bpos, unsigned int tile_size)
{
	...
	pos[0] = player->xpos * tile_size - (player_size / 2) + bpos[0];
	pos[1] = player->ypos * tile_size - (player_size / 2) + bpos[1];
	if (player_size == 0)
		player_size = 1;
